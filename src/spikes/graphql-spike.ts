...
function buildQuery(slugs: readonly RepoSlug[]): string {
  const fields = slugs
    .map((s, i) => {
      const alias = `r${i}`;
...
async function main(): Promise<void> {
  ...
  const slugs: RepoSlug[] = sampleNames.map((name) => ({ owner, name }));
  ...
  for (let i = 0; i < slugs.length; i += BATCH) {
    ...
    const result = (await ghGraphql(query)) as {
      data?: Record<string, {
        nameWithOwner?: string;
        defaultBranchRef?: { name?: string; target?: { oid?: string } } | null;
        pullRequests?: { nodes?: Array<{ number: number; isDraft: boolean }> };
      } | null>;
    ...
    for (const key of Object.keys(data)) {
      ...
      if (prs.length > 0) openPrs += 1;
      console.log(
        `  ${(repo.nameWithOwner ?? "?").padEnd(40)} ${branch.padEnd(10)} ${sha.slice(0, 7)} ${prs.length > 0 ? `PR#${prs[0]?.number}` : ""}`,
      );
    }
  }
  ...
  console.log(
    `H2 verdict (calls = ceil(N/${BATCH})): ${calls === expectedCalls && resolved === slugs.length ? "PASS" : "FAIL"}`,
  );
