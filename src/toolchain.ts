...
/**
 * `.luma/toolchain.yml` — declarative spec for the local machine's dev
 * toolchain, consumed by `luma setup` / `luma doctor` / `luma fix`. Mirrors
 * the `.luma/stacks.json` pattern from `up.ts`, but YAML (via the existing
 * `src/yaml.ts` parser) since the shape the user wants —
 *
 *   sdk:
 *     cmake: latest
 *     java: "21"
 ...
 *     vscode: true
 *
 * — is exactly the nested-map/scalar subset that parser already supports.
 */

export type ToolchainGroup = "sdk" | "tools";

export interface ToolchainEntry {
  id: string;
  group: ToolchainGroup;
  /** `true` (or the literal "latest") means "installed, any version is fine". */
  desired: string | true;
}

export interface LoadedToolchain {
  workspaceRoot: string;
  configPath: string;
  entries: ToolchainEntry[];
}

const GROUPS: ToolchainGroup[] = ["sdk", "tools"];
...
export function loadToolchainConfig(): LoadedToolchain {
  ...
  const doc = parsed as Record<string, YamlValue>;

  for (const key of Object.keys(doc)) {
    if (!GROUPS.includes(key as ToolchainGroup)) {
  ...
  for (const group of GROUPS) {
    ...
    for (const [id, value] of Object.entries(raw as Record<string, YamlValue>)) {
      if (seen.has(id)) {
...
function normalizeDesired(value: YamlValue, configPath: string, id: string): string | true {
...
export function selectEntries(entries: ToolchainEntry[], names: string[]): ToolchainEntry[] {
  if (names.length === 0) return entries;
  const byId = new Map(entries.map((e) => [e.id, e]));
  const chosen: ToolchainEntry[] = [];
