...
function isPlanType(s: string): s is PlanType {
  return (PLAN_TYPES as readonly string[]).includes(s);
}

function planFolder(repoRoot: string, type: PlanType): string {
...
function template(id: string, type: PlanType, title: string): string {
  return `---
id: ${id}
type: ${type}
status: proposed
...
function planNew(args: string[]): number {
  ...
  if (!isPlanType(type)) {
    process.stderr.write(
      `${BIN_NAME} plan new: unknown type '${typeArg}' (expected CTX|EXP|PRD|PROP|ADR)\n`,
    );
    return 1;
  }
...
function planList(args: string[]): number {
  const filterArg = args[0];
  let filter: PlanType | undefined;
  if (filterArg !== undefined) {
    const upper = filterArg.toUpperCase();
    if (!isPlanType(upper)) {
      process.stderr.write(
        `${BIN_NAME} plan list: unknown type '${filterArg}' (expected CTX|EXP|PRD|PROP|ADR)\n`,
      );
      return 1;
    }
    filter = upper;
  ...
  if (printed === 0) process.stdout.write("  (no plans found)\n");
  return 0;
}
