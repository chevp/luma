...
interface Issue {
  kind: "no-gitignore" | "missing-patterns" | "tracked-artifacts" | "broken-gitlink";
  detail: string[];
}
...
function gitLsFilesWithMode(root: string): { mode: string; path: string }[] {
  const r = execSync("git", ["-C", root, "ls-files", "-s"]);
  if (!r.ok) return [];
  const out: { mode: string; path: string }[] = [];
  for (const ln of r.stdout.split(/\r?\n/)) {
    if (!ln) continue;
    // Format: <mode> <object> <stage>\t<path>
    const m = ln.match(/^(\d{6})\s+\S+\s+\d+\t(.+)$/);
    if (m && m[1] && m[2]) out.push({ mode: m[1], path: m[2] });
  }
  return out;
}

function detectStack(tracked: string[]): Stack {
  const stack: Stack = {
    maven: false,
    gradle: false,
    node: false,
  ...
  for (const p of tracked) {
    ...
    if (name === "pom.xml") stack.maven = true;
    else if (
      name === "build.gradle" ||
      ...
      stack.gradle = true;
    else if (name === "package.json" && !p.includes("node_modules/")) stack.node = true;
    else if (name === "pyproject.toml" || name === "setup.py" || name === "requirements.txt")
      stack.python = true;
    else if (name === "AndroidManifest.xml" || name === "local.properties") stack.android = true;
  }
  if (stack.android) stack.gradle = true;
  return stack;
}
...
function buildGitignore(stack: Stack): string {
  const sections: string[][] = [];
  if (stack.maven) sections.push(["# --- Maven ---", ...PATTERNS_MAVEN]);
  if (stack.gradle) sections.push(["# --- Gradle ---", ...PATTERNS_GRADLE]);
  if (stack.android) sections.push(["# --- Android ---", ...PATTERNS_ANDROID]);
  if (stack.node) sections.push(["# --- Node ---", ...PATTERNS_NODE]);
  if (stack.python) sections.push(["# --- Python ---", ...PATTERNS_PYTHON]);
  sections.push(["# --- IDE ---", ...PATTERNS_IDE]);
  sections.push(["# --- OS ---", ...PATTERNS_OS]);
  return sections.map((s) => s.join("\n")).join("\n\n") + "\n";
}

function expectedPatternsFor(stack: Stack): string[] {
  const out: string[] = [];
  if (stack.maven) out.push(...PATTERNS_MAVEN);
  if (stack.gradle) out.push(...PATTERNS_GRADLE);
  if (stack.android) out.push(...PATTERNS_ANDROID);
  if (stack.node) out.push(...PATTERNS_NODE);
  if (stack.python) out.push(...PATTERNS_PYTHON);
  return out.filter((p) => !p.startsWith("!"));
}

// Build-output paths that should never be tracked, regardless of stack.
const ARTIFACT_RULES: { name: string; test: (p: string) => boolean }[] = [
...
function findTrackedArtifacts(tracked: string[]): { rule: string; paths: string[] }[] {
...
function parseGitmodulesPaths(content: string): Set<string> {
...
function findBrokenGitlinks(root: string): string[] {
  const entries = gitLsFilesWithMode(root).filter((e) => e.mode === "160000");
  if (entries.length === 0) return [];
  const gmPath = join(root, ".gitmodules");
  const declared = existsSync(gmPath) ? parseGitmodulesPaths(readFileSync(gmPath, "utf8")) : new Set<string>();
...
function mergeGitignore(existing: string, stack: Stack): { merged: string; added: string[] } {
...
export async function run(argv: string[]): Promise<number> {
  ...
  if (stack.maven) stackLabels.push("maven");
  if (stack.gradle) stackLabels.push("gradle");
  if (stack.android) stackLabels.push("android");
  if (stack.node) stackLabels.push("node");
  if (stack.python) stackLabels.push("python");
  ...
  if (artifacts.length === 0) {
    ...
    for (const b of artifacts) {
      ...
      for (const p of b.paths.slice(0, 3)) info(`  ${p}`);
      if (b.paths.length > 3) info(`  ... and ${b.paths.length - 3} more`);
    }
    issues.push({
  ...
  for (const iss of issues) {
    if (iss.kind === "no-gitignore") {
      ...
      if (await confirm("  untrack these paths?", autoYes)) {
        ...
        if (failed === 0) ok(`untracked ${iss.detail.length} path(s) — commit to record removal`);
        else fail(`${failed} path(s) failed to untrack`);
      } else {
