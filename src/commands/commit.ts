...
const META_TITLE_RE =
  /^\s*(i\b|i'?ll\b|i'?m\b|i'?ve\b|we\b|here\b|here'?s\b|below\b|sure\b|okay\b|ok\b|let\b|let'?s\b|this\b|that\b|these\b|those\b|first\b|now\b|alright\b|response\b|summary\b|in\s+summary\b|to\s+summarize\b|the\s+(diff|change|changes|code|provided|following|patch|file|files|original)\b)/i;

const ECHOED_CODE_RE =
  /^\s*(import\s|export\s|class\s+\w|function\s+\w|const\s+\w+\s*=|let\s+\w+\s*=|var\s+\w+\s*=|def\s+\w|public\s|private\s|return\s|if\s*\(|<\w)/;

const META_REFERENCE_RE =
  /\b(provided\s+code|code\s+example|the\s+diff|the\s+code|attached|the\s+following\s+code|original\s+code)\b/i;

function looksLikeMeta(title: string): boolean {
  if (!title) return false;
  if (META_TITLE_RE.test(title)) return true;
  if (ECHOED_CODE_RE.test(title)) return true;
  if (META_REFERENCE_RE.test(title)) return true;
  return false;
}
...
function cleanupMessage(raw: string): string {
  ...
  if (looksLikeMeta(stripped[0] ?? "")) return "";
