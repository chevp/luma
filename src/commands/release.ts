...
const FLAGS: Record<string, (a: Args) => void> = {
...
function parseArgs(argv: string[]): Args | { error: string } {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "-m" || a === "--message" || a === "--asset" || a === "--remote") {
      ...
      if (a === "--asset") out.assets.push(v);
      else if (a === "--remote") out.remote = v;
      else out.message = v;
      continue;
    }
    if (a.startsWith("-")) return { error: `unknown option '${a}'` };
    if (out.version !== null || out.level !== null) return { error: `unexpected argument '${a}'` };
    if (a === "patch" || a === "minor" || a === "major") out.level = a;
    else out.version = a.replace(/^v/i, "");
  }
  return out;
}
...
export async function run(argv: string[]): Promise<number> {
  ...
  if (!commandExists("git")) return fail("missing dependency: git");
  if (!isInsideRepo()) return fail("not a git repository");
  if (args.ghRelease && !commandExists("gh")) return fail("--gh-release needs the gh CLI");
  ...
  let target: string;
  let level: BumpLevel | null = args.level;
  let llmNote = "";
  if (args.version) {
    target = args.version;
  } else if (args.noBump) {
    target = primary as string;
  } else {
    if (!level) {
      const auto = args.noLlm ? null : await llmReview(root, previousTag, null, skill);
      level = auto?.level ?? "patch";
      if (auto && level === "major" && primary?.startsWith("0.")) level = "minor";
      llmNote = auto ? `${level} — ${auto.reason}` : "";
      if (!auto) process.stdout.write(`${sym.warn} no LLM answer, defaulting to patch\n`);
    }
    const next = nextSemver(primary as string, level);
    if (!next) return fail(`current version '${primary}' is not parseable semver`);
    target = next;
  }
  ...
  if (llmNote) process.stdout.write(`${sym.ok} LLM level: ${llmNote}\n`);
  for (const w of checks.warnings) process.stdout.write(`${sym.warn} ${c.yellow(w)}\n`);
  for (const e of checks.errors) process.stdout.write(`${sym.err} ${c.red(e)}\n`);
  if (checks.errors.length) return 1;

  if (git(["rev-parse", "--verify", "--quiet", `refs/tags/${tag}`]).ok) return fail(`tag ${tag} already exists`);
  const counts = porcelain();
  if (counts.total > 0) {
    return fail(
      `working tree has uncommitted changes (${counts.staged} staged · ${counts.unstaged} unstaged · ${counts.untracked} untracked)\ncommit or stash them before tagging`,
    );
  }
  ...
  if (toWrite.length && bumpFrom !== null) {
    ...
    if (!add.ok) return fail(add.stderr || "failed to stage version files");
    const commit = git(["commit", "-m", `release ${tag}`], root);
    if (!commit.ok) return fail(commit.stderr || "failed to commit version bump");
  ...
  if (!tagRes.ok) return fail(tagRes.stderr || `failed to create tag ${tag}`);
  ...
  if (code !== 0) return fail(`gh release create failed (exit ${code}); tag ${tag} is already pushed`);
  return 0;
}
