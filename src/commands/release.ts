...
function parseArgs(argv: string[]): Args | { error: string } {
  const out: Args = { version: null, message: null, push: true, remote: "origin" };
  for (let i = 0; i < argv.length; i++) {
    ...
    if (out.version !== null) return { error: `unexpected argument '${a}'` };
    out.version = a;
  }
  return out;
}
...
export async function run(argv: string[]): Promise<number> {
  ...
  if (!tag) {
    process.stderr.write(
      `${BIN_NAME} release: '${versionArg}' is not a valid semver version (expected X.Y.Z)\n`,
    );
    return 1;
  }
