...
function readCache(): Cache | null {
  ...
  try {
    const raw = JSON.parse(readFileSync(CACHE_FILE, "utf8")) as Cache;
    if (typeof raw.checkedAt !== "string" || typeof raw.latest !== "string") return null;
    return raw;
  } catch {
    return null;
  }
...
function isCacheFresh(cache: Cache | null): boolean {
...
export async function maybePrintUpdateNotice(): Promise<void> {
  if (process.env.LUMA_NO_VERSION_CHECK === "1") return;
  ...
  if (compareVersions(cache.latest, current) <= 0) return;
