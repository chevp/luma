...
function ensureBackupDir(repoRoot, report) {
    if (report.backupDir)
    ...
    try {
        ...
        report.errors.push(`could not create backup dir ${abs}: ${e.message}`);
        return null;
    }
    report.backupDir = abs;
    return abs;
}
function backupFile(repoRoot, rel, report) {
    ...
    try {
        ...
        report.errors.push(`backup ${rel} failed: ${e.message}`);
        return null;
    }
}
function backupBlob(repoRoot, name, content, report) {
    const root = ensureBackupDir(repoRoot, report);
    if (!root)
        return null;
    ...
    try {
        ...
        report.errors.push(`backup ${name} failed: ${e.message}`);
        return null;
    }
}
function defaultBranch(repoRoot) {
    ...
    for (const cand of ["main", "master"]) {
        if (git(["-C", repoRoot, "show-ref", "--verify", "--quiet", `refs/remotes/origin/${cand}`]).ok ||
 ...
 * make progress.
 */
function recoverDetachedHead(repoRoot, report) {
 ...
 */
function clearStashes(repoRoot, report) {
 ...
 */
function resolveConflictMarkers(repoRoot, report) {
 ...
 */
function unstageEmbeddedRepos(repoRoot, report) {
    ...
    for (const [path, kind] of candidates) {
        backupBlob(repoRoot, `embedded/${path.replace(/[/\\]/g, "_")}.txt`, `path=${path}\nkind=${kind}\nnote=git rm --cached executed; directory left in place\n`, report);
...
export function recoverRepo(repoRoot, opts = {}) {
    const report = {
    ...
    const inRebase = existsSync(join(repoRoot, dir, "rebase-merge")) ||
...
export function isClean(report) {
    return (report.detachedHead === null &&
...
export function printReport(report) {
    ...
    if (report.detachedHead) {
        ...
        process.stdout.write(`  ${c.green("✓")} detached HEAD recovered: ${from.slice(0, 8)} → ${to}\n`);
    }
...
export function summarize(reports) {
    ...
    if (totals.errors > 0)
        parts.push(c.red(`${totals.errors} error${totals.errors === 1 ? "" : "s"}`));
    return parts.join(", ");
}
