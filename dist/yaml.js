...
function parseMap(lines, start, blockIndent) {
    const out = {};
    let i = start;
...
function parseList(lines, start, blockIndent) {
    const out = [];
    let i = start;
    while (i < lines.length) {
        const ln = lines[i];
        ...
        if (after === "") {
            ...
            const synthetic = { no: ln.no, indent: childIndent, text: " ".repeat(childIndent) + after };
            const block = [synthetic];
            let j = i + 1;
            while (j < lines.length && lines[j].indent >= childIndent) {
                const cand = lines[j];
                if (cand.indent === blockIndent && cand.text.trimStart().startsWith("-"))
                    break;
                block.push(cand);
                j += 1;
            }
...
function parseInlineList(body, lineNo) {
    const items = [];
    let depth = 0;
    let inSingle = false;
    let inDouble = false;
...
export function getPath(doc, expr) {
    const tokens = [];
    ...
    while ((m = re.exec(expr)) !== null) {
        if (m[1] !== undefined)
            ...
            tokens.push({ idx: Number.parseInt(m[2], 10) });
    }
    let cur = doc;
