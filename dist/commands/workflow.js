...
import { c } from "../ui.js";
import { createInputs, findWorkflowsDir, inputDecls, loadFile, planSteps, resolveWorkflow, validate, } from "../workflow/loader.js";
...
function cmdShow(argv) {
    ...
    try {
        ...
        process.stderr.write(`${BIN_NAME} workflow: ${err.message}\n`);
        return 1;
    }
    ...
    if (desc)
        process.stdout.write(`${c.bold("description")} ${desc}\n`);
    const t = getPath(doc, ".trigger");
    if (Array.isArray(t)) {
...
function parseRun(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a.startsWith("--")) {
            ...
            const v = argv[++i];
            if (v === undefined)
                return { error: `missing value for --${k}` };
            o.inputs.push([k, v]);
            continue;
        }
        if (a.startsWith("-"))
...
async function cmdRun(argv) {
    ...
    if ("help" in parsed) {
        process.stdout.write(`chi workflow run — execute a workflow's steps.
...
`);
    ...
    for (const [k, v] of parsed.inputs)
        inputs.set(k, v);
    // Verify required inputs.
    const missing = [];
    for (const decl of inputDecls(resolved.doc)) {
        if (decl.required && !inputs.has(decl.name))
...
export async function runAlias(argv) {
    return cmdRun(argv);
}
