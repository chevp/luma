...
function markerPath(worktreePath, common) {
...
function defaultRoot(repoRootPath) {
...
function deriveWorktreePath(name, repoRootPath) {
 ...
 */
export function listActiveChiFlows() {
    if (!isInsideRepo())
        return [];
 ...
 */
export function detectChiWorktree() {
    if (!isInsideRepo())
        return null;
    ...
    if (resolve(absGd) === resolve(absCd))
        return null;
...
export function createWorktree(name, opts = {}) {
    ...
    try {
        writeFileSync(marker, `name=${name}\n` +
            `branch=${branch}\n` +
            `base=${base}\n` +
            `source=${root}\n` +
...
async function cmdCreate(name, base) {
...
async function cmdList() {
    ...
    for (const w of wts) {
        ...
        process.stdout.write(`  ${c.cyan(display.padEnd(20))} ${branch.padEnd(28)} ${state}  ${c.dim(w.path)}\n`);
    }
    return 0;
}
function findWorktreeByName(name, wts, repoBase) {
...
async function cmdRm(name, force) {
    ...
    const runFrom = here && resolve(here) === resolve(target.path) && meta?.source ? meta.source : undefined;
