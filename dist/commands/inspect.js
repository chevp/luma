...
import { discoverRepos, repoLabel, resolveWorkspaceRoot, isRepo, } from "../workspace.js";
...
function isBuiltPath(relPath) {
    for (const seg of relPath.split("/")) {
        if (EXCLUDED_DIRS.has(seg))
            return true;
        if (seg.startsWith("cmake-build"))
            return true;
    }
    return false;
}
...
function countLines(abs, relPath) {
    const e = ext(relPath);
    if (BINARY_EXT.has(e) || NON_CODE_EXT.has(e) || isGenerated(relPath))
        return 0;
    let buf;
    try {
...
async function visibilityOf(root, lookup) {
    if (!lookup)
        return "—";
    const r = await execAsync("gh", ["repo", "view", "--json", "visibility", "--jq", ".visibility"], { cwd: root, timeoutMs: 8000 });
    if (!r.ok)
        return "—";
...
async function measure(info, lookupVis) {
    const tracked = (await trackedFiles(info.path)).filter((rel) => !isBuiltPath(rel));
    let loc = 0;
    for (const rel of tracked)
        loc += countLines(join(info.path, rel), rel);
...
const BAR_W = 20;
function sizeBar(loc, maxLoc, subs) {
...
function renderTable(rows) {
    ...
    const head = "REPOSITORY".padEnd(repoW) +
        "  " + "VIS".padEnd(VIS) +
        "  " + "LOC".padStart(LOC) +
    ...
    for (const r of rows) {
        const line = r.label.padEnd(repoW) +
...
export async function run(argv) {
    ...
    if (repos.length === 0) {
        process.stderr.write(`${BIN_NAME} inspect: no git repositories found under ${ws.root.replace(/\\/g, "/")}\n`);
        return 1;
    }
