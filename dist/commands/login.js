...
async function loginClaude() {
    ...
    try {
        ...
        process.stderr.write(`${BIN_NAME} login: ${err instanceof Error ? err.message : String(err)}\n`);
        return 1;
    }
    ...
    process.stdout.write(`  ${c.green("✓")} saved github_copilot_token + provider=claude to ${CHI_CONFIG_FILE}\n`);
    ...
    if (await claudeProvider.ping()) {
        process.stdout.write(`  ${c.green("✓")} reachable — model: ${c.cyan(claudeProvider.activeModel())}\n`);
    }
    else {
        process.stdout.write(`  ${c.yellow("!")} authenticated, but no Claude model is available via your Copilot plan\n`);
    }
