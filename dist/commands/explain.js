...
export async function run(argv) {
    ...
    const prompt = "You are diagnosing a failed git or gh command for a developer.\n\n" +
    ...
    if (!(await provider.ping())) {
        process.stderr.write(`${BIN_NAME} explain: provider '${activeProviderName()}' not reachable\n` +
            `             run '${BIN_NAME} doctor provider' for diagnostics\n`);
        return 1;
    }
    let answer = "";
    try {
        answer = await withSpinner(`diagnosing via ${activeProviderName()} (${provider.activeModel()})`, () => provider.generate(prompt));
    }
    catch (err) {
