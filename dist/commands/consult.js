...
import { getOrchestrator, MissingAnthropicKeyError, } from "../orchestrator/index.js";
...
function parseArgs(argv) {
    const out = { write: false, allowBash: false, showHelp: false };
    const positional = [];
...
function pickAllowedTools(write, allowBash) {
    const base = ["Read", "Glob", "Grep", "mcp__chi__ask_user", "mcp__chi__plan_list"];
    if (write)
        base.push("Edit", "Write", "mcp__chi__plan_create");
    if (allowBash)
        base.push("Bash");
    return base;
}
function pickPermissionMode(write, allowBash) {
    if (allowBash)
...
export async function run(argv) {
    ...
    if (parsed.allowBash && !parsed.write) {
        process.stderr.write(`${BIN_NAME} consult: --dangerously-allow-bash requires --write (refusing to enable Bash without write)\n`);
        return 1;
    }
    ...
    process.stdout.write(`${c.dim(`(${ws.mode} mode at ${ws.root}, permission=${opts.permissionMode})`)}\n`);
    ...
    try {
        ...
        if (err instanceof MissingAnthropicKeyError) {
            process.stderr.write(`${BIN_NAME} consult: ANTHROPIC_API_KEY is not set — run \`${BIN_NAME} init --provider=claude\` or export the env var\n`);
            return 1;
        }
        throw err;
    }
    ...
    if (final.state === "terminated.error") {
        process.stderr.write(`${BIN_NAME} consult: ${final.error?.message ?? "orchestrator terminated with error"}\n`);
        return 1;
    }
