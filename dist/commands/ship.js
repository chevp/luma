 ...
 */
function resolveSubmoduleBranch(repoRoot, smName, smAbs) {
    // `.` is a special .gitmodules value ("track the superproject's branch") —
    // treat it as unset and fall through to the remote default.
    const configured = git(["-C", repoRoot, "config", "-f", ".gitmodules", "--get", `submodule.${smName}.branch`]).stdout.trim();
    ...
    if (git(["-C", smAbs, "remote", "set-head", "origin", "-a"]).ok) {
        ...
        if (head2.startsWith(PREFIX))
            return head2.slice(PREFIX.length);
    }
    return null;
}
/**
 ...
 */
function attachAndPullSubmodule(repoRoot, smPath, smName, smAbs) {
    if (!git(["-C", smAbs, "symbolic-ref", "-q", "HEAD"]).ok) {
        ...
        if (!branch) {
            process.stdout.write(`${sym.warn} ${c.dim(`${BIN_NAME} ship:`)} ${c.cyan(smPath)} is in detached HEAD and no default branch could be resolved, ${c.dim("skipping pull")}\n`);
            return;
        }
        if (!git(["-C", smAbs, "checkout", branch]).ok) {
            process.stdout.write(`${sym.warn} ${c.dim(`${BIN_NAME} ship:`)} ${c.cyan(smPath)} detached — could not checkout ${c.cyan(branch)} ${c.dim("(skipping pull)")}\n`);
            return;
        }
        process.stdout.write(`${sym.ok} ${c.dim(`${BIN_NAME} ship:`)} ${c.cyan(smPath)} ${c.dim(`was detached → checked out ${branch}`)}\n`);
    }
    ...
    for (const { smPath, smName, registered } of entries) {
        ...
        if (!existsSync(join(smAbs, ".git"))) {
            if (dry) {
                ...
                process.stdout.write(`${sym.warn} ${c.dim(`${BIN_NAME} ship:`)} ${c.cyan(smPath)} is an embedded gitlink not checked out and not in .gitmodules ${c.dim("(no URL to init from) — skipping")}\n`);
            }
            continue;
        }
        ...
        if (dry) {
            if (git(["-C", smAbs, "symbolic-ref", "-q", "HEAD"]).ok) {
                ...
                dryNote(`would re-attach detached ${c.cyan(smPath)} to its default branch + pull --ff-only`);
            }
        }
        else {
