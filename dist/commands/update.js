 ...
 */
async function selfUpdateFromWorkspaceClone(root, oldVersion) {
    const upstream = git(["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], root).stdout.trim();
    if (!upstream) {
        process.stderr.write(`${BIN_NAME} update: workspace clone has no upstream — set one with \`git branch --set-upstream-to=...\`\n`);
        return 1;
    }
    const pull = await execInherit("git", ["pull", "--ff-only", "--autostash"], { cwd: root });
    if (pull !== 0) {
        process.stderr.write(`${BIN_NAME} update: git pull failed in ${root} — resolve manually and retry\n`);
        return pull;
    }
 ...
 */
async function selfUpdateFromGithub() {
    clearStaleGlobalSymlink();
